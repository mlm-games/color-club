...
static func rect_to_path_string(x: float, y: float, w: float, h: float, rx: float, ry: float) -> String:
	...
	var path = "M %f %f " % [x + rx, y]
	path += "L %f %f " % [x + w - rx, y]
	path += "A %f %f 0 0 1 %f %f " % [rx, ry, x + w, y + ry]
	path += "L %f %f " % [x + w, y + h - ry]
	path += "A %f %f 0 0 1 %f %f " % [rx, ry, x + w - rx, y + h]
	path += "L %f %f " % [x + rx, y + h]
	path += "A %f %f 0 0 1 %f %f " % [rx, ry, x, y + h - ry]
	path += "L %f %f " % [x, y + ry]
	path += "A %f %f 0 0 1 %f %f " % [rx, ry, x + rx, y]
	path += "Z"
...
# --- Arc Tessellation ( from GodSVG) ---
static func tessellate_elliptical_arc(p1: Vector2, rx: float, ry: float, phi_deg: float,
		fA: bool, fS: bool, p2: Vector2) -> PackedVector2Array:
	if p1.is_equal_approx(p2) or rx <= 0 or ry <= 0:
		return [p2]
	...
	var rx_sq = rx * rx
	var ry_sq = ry * ry
	var p1p_x_sq = p1_prime.x * p1_prime.x
	...
	var c_factor = sign * sqrt(num / den) if den > 0 else 0.0
	...
	if not fS and delta_angle > 0:
		delta_angle -= TAU
	elif fS and delta_angle < 0:
		delta_angle += TAU
